...
func (r *Ring) Dequeue() byte {
	r.checkInit()
	if r.Length() == 0 {
		panic("Do not peek into an empty ring buffer")
	}
	v := r.get(r.tail)
	if r.tail == r.head {
		r.Empty()
...
*/
func (r *Ring) Values() ([]byte, []byte) {
	if r.Length() == 0 {
		return []byte{}, []byte{}
	}
	if r.tail > r.head {
		return r.buff[r.tail:], r.buff[:r.head+1]
	} else {
		return r.buff[r.tail:r.head + 1], []byte{}
	}
}
...
/*
Discards buffer's contents without reallocation
*/
func (r *Ring) Empty() {
	if r.Length() > 0 {
		r.head = -1
...
func (r *Ring) checkInit() {
	if r.buff == nil {
		r.buff = make([]byte, DefaultCapacity)
		r.head, r.tail = -1, 0
	}
}
